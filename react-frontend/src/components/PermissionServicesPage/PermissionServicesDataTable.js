...
const PermissionServicesDataTable = ({
    ...
    const p_booleanTemplate3 = (rowData, { rowIndex }) => <p>{String(rowData.readAll)}</p>;
    const p_booleanTemplate4 = (rowData, { rowIndex }) => <p>{String(rowData.updateOwn)}</p>;
    const p_booleanTemplate5 = (rowData, { rowIndex }) => <p>{String(rowData.updateAll)}</p>;
    const p_booleanTemplate6 = (rowData, { rowIndex }) => <p>{String(rowData.deleteOwn)}</p>;
    const p_booleanTemplate7 = (rowData, { rowIndex }) => <p>{String(rowData.deleteAll)}</p>;
    ...
    return (
        <>
            ...
            >
                ...
                <Column field="read" header="Read own" body={p_booleanTemplate2} filter={selectedFilterFields.includes('read')} hidden={selectedHideFields?.includes('read')} sortable style={{ minWidth: '8rem' }} />
                <Column field="readAll" header="Read all" body={p_booleanTemplate3} filter={selectedFilterFields.includes('readAll')} hidden={selectedHideFields?.includes('readAll')} sortable style={{ minWidth: '8rem' }} />
                <Column field="updateOwn" header="Update own" body={p_booleanTemplate4} filter={selectedFilterFields.includes('updateOwn')} hidden={selectedHideFields?.includes('updateOwn')} sortable style={{ minWidth: '8rem' }} />
                <Column field="updateAll" header="Update all" body={p_booleanTemplate5} filter={selectedFilterFields.includes('updateAll')} hidden={selectedHideFields?.includes('updateAll')} sortable style={{ minWidth: '8rem' }} />
                <Column field="deleteOwn" header="Delete own" body={p_booleanTemplate6} filter={selectedFilterFields.includes('deleteOwn')} hidden={selectedHideFields?.includes('deleteOwn')} sortable style={{ minWidth: '8rem' }} />
                <Column field="deleteAll" header="Delete all" body={p_booleanTemplate7} filter={selectedFilterFields.includes('deleteAll')} hidden={selectedHideFields?.includes('deleteAll')} sortable style={{ minWidth: '8rem' }} />
