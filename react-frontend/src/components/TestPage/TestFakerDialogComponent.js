...
import testFakerFactory from "./testFakerFactory";
...
const TestFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = testFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("test").create(elem));
...
export default connect(mapState, mapDispatch)(TestFakerDialogComponent);
