...
const MailQuesPage = (props) => {
    ...
    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'mailQues' });
        if (!service) {
            service = _.find(standard.services, { serviceName: 'mailQues' });
        }
    ...
    const deleteRow = async () => {
        try {
            await client.service('mailQues').remove(data[selectedEntityIndex]?._id);
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Mail Ques </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="mailQues-add-button" />
            ...
            <MailQuesEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MailQuesCreateDialogComponent show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
