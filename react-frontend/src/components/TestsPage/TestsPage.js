...
const TestsPage = (props) => {
    ...
    useEffect(() => {
        const service = _.find(config.services, { serviceName: 'tests' });
    ...
    const deleteRow = async () => {
        try {
            await client.service('tests').remove(data[selectedEntityIndex]?._id);
    ...
    return (
        <div className="mt-5">
            ...
            <TestsEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TestsCreateDialogComponent show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
