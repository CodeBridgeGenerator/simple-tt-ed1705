...
const CompaniesCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        const init = { companyNo: 'Cas', isdefault: false };
        set_entity({ ...init });
    }, [props.show]);
    ...
    useEffect(() => {
        ...
        client
            .service('companyAddresses')
            .find({ query: { $limit: 10000, $sort: { createdAt: -1 }, _id: urlParams.singleCompanyAddressesId } })
            .then((res) => {
                setAddresses(
                    res.data.map((e) => {
    ...
    const addressesOptions = addresses.map((elem) => ({ name: elem.name, value: elem.value }));
