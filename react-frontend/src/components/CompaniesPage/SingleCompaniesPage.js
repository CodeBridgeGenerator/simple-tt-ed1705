...
const SingleCompaniesPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const addresses = Array.isArray(res.addresses) ? res.addresses.map((elem) => ({ _id: elem._id, Street1: elem.Street1 })) : res.addresses ? [{ _id: res.addresses._id, Street1: res.addresses.Street1 }] : [];
