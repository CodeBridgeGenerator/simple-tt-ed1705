...
const TemplatesPage = (props) => {
    ...
    useEffect(() => {
        const service = _.find(config.services, { serviceName: 'templates' });
    ...
    const deleteRow = async () => {
        try {
            await client.service('templates').remove(data[selectedEntityIndex]?._id);
    ...
    return (
        <div className="mt-5">
            ...
            <TemplatesEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TemplatesCreateDialogComponent show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
