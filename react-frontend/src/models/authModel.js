    ...
    effects: (dispatch) => ({
        ...
        async changeUserPassword({ oldPassword, newPassword }, reduxState) {
            return new Promise(async (resolve, reject) => {
                ...
                await client
                    .service('users')
                    .patch(reduxState.auth.user._id, { oldPassword, newPassword, changePassword: true, clientName: 'codebridge-website' })
                    .then((res) => {
                        dispatch.toast.alert({ type: 'success', title: 'Password', message: 'User password updated successfully!' });
                        resolve();
                    })
                    .catch((err) => {
                        console.log('Failed to update user password', err);
                        dispatch.toast.alert({ type: 'error', title: 'Password', message: err.message || 'Failed to update user password' });
                        this.update({ passwordPolicyErrors: Array.isArray(err.data) ? err.data : [] });
                        reject(err);
                    });
